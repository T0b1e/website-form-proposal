document.addEventListener('DOMContentLoaded', () => {
    ...
    function fetchTaskData(searchCriteria = '', searchTerm = '', page = 1, limit = 10) {
        ...
        loadingMessage.innerHTML = `<td colspan="9" style="text-align:center;">กำลังโหลด...</td>`; // Adjust colspan to 9
        ...
        fetch(url)
            ...
            .then(data => {
                ...
                if (data.success) {
                    populateTable(data.tasks); // Pass only tasks to populateTable
                    recordCountLabel.textContent = `📊 จำนวนบันทึกทั้งสิ้น: ${data.total}`;
            ...
            .catch(error => {
                ...
                errorRow.innerHTML = `<td colspan="9" style="text-align:center; color: red;">เกิดข้อผิดพลาดในการโหลดข้อมูล</td>`; // Adjust colspan to 9
    ...
    function createTaskRow(task) {
        ...
        row.innerHTML = `
            ...
            <td>${task.instructions || 'ไม่พบเจอ'}</td>
            <td>${task.remarks || 'ไม่พบเจอ'}</td>
            <td>
